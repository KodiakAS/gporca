...
def parseInputFile(inputFile):
	failed_tests = []
	with open(inputFile, "r", encoding="utf-8", errors="replace") as fp:
		for line in fp:
...
def processLogFile(logFileLines):
	...
	for line in logFileLines:
		fileNameMatch = re.search(r'(\.\./data/dxl/minidump/[^"\s]+\.mdp)', line)
		...
		if fileNameMatch:
			current_file = fileNameMatch.group(1)
			if read_plan > 0:
...
def processExistingLogFile(logFileName):
	with open(logFileName, "r", encoding="utf-8", errors="replace") as fp:
		processLogFile(fp)
